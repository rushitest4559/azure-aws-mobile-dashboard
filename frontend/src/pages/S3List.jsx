...
import { FaDatabase, FaSync, FaExclamationTriangle, FaRobot, FaSpinner } from 'react-icons/fa';
...
const S3List = () => {
    ...
    const generateAISummary = async () => {
        ...
        try {
            // Initialize Gemini AI inside the function to ensure API key is available
            const genAI = new GoogleGenerativeAI(import.meta.env.VITE_GEMINI_API_KEY);

            // Prepare data from TanStack Query - buckets is already available from useQuery
            const bucketData = buckets.map(b => ({
                name: b.name,
                region: b.region || 'us-east-1',
                created: new Date(b.created).toLocaleDateString()
            }));

            // Create detailed prompt
            const prompt = `Analyze these AWS S3 buckets and provide exactly 2-3 key insights (each insight should be one concise sentence under 20 words):
...
- Naming patterns or organization issues
- Any security or cost optimization suggestions
...
3. Third insight here`;
            ...
            if (insights.length > 0) {
                ...
                setAiSummary([
                    text.trim() || `${buckets.length} S3 buckets analyzed successfully`,
                    'Review your bucket configurations for optimization opportunities',
                    'Consider implementing consistent naming conventions'
                ]);
            }
            ...
            const uniqueRegions = new Set(buckets.map(b => b.region || 'us-east-1')).size;
            const totalBuckets = buckets.length;

            setAiSummary([
                `Total of ${totalBuckets} S3 bucket${totalBuckets !== 1 ? 's' : ''} across ${uniqueRegions} region${uniqueRegions !== 1 ? 's' : ''}`,
                'Consider implementing lifecycle policies for automated cost optimization',
                'Review bucket access policies to ensure proper security configuration'
            ]);
        } finally {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
                {/* AI Summary Section - Only show when buckets exist */}
                {buckets.length > 0 && (
                    <div className="mb-6">
                        {!showSummary ? (
                            <button
                ...
                {isError && (
                    <div className="bg-red-50 border border-red-100 rounded-2xl p-4 flex items-start gap-3 mb-6">
                        ...
                        <div className="flex-1 text-sm text-red-800">
                            <p className="font-medium mb-1">Unable to fetch buckets</p>
                ...
                {/* Empty State */}
                {!isFetching && buckets.length === 0 && !isError ? (
                    <div className="text-center py-16 sm:py-20">
                        ...
                        <p className="text-sm text-gray-500">Tap refresh to sync your buckets</p>
                    </div>
                ) : (
                    <>
                        {/* Desktop Table View */}
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {buckets.map(b => (
                                        <tr
                                            key={b.name}
                                            onClick={() => navigate(`/aws/details/${b.name}`)}
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 text-gray-600">
                                                {new Date(b.created).toLocaleDateString(undefined, {
                                                    year: 'numeric',
                                                    month: 'short',
                                                    day: 'numeric'
                                                })}
                                ...
                                >
                                    ...
                                    <div className="px-4 py-3.5 border-b border-gray-50">
                                        <div className="flex items-start justify-between gap-3">
                                            ...
                                            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-orange-50 text-orange-700 flex-shrink-0">
                                                AWS
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="px-4 py-3 space-y-2">
                                        <div className="flex items-center justify-between text-sm">
                                            ...
                                            <span className="font-medium text-gray-900">{b.region || 'us-east-1'}</span>
                                        </div>
                                        <div className="flex items-center justify-between text-sm">
